    ...
    try:
        ...
        try:
            ...
            if r.returncode == 2:
                ...
                # ⭐ 4 = 報告已產出,但**有暫存沒收乾淨**(Codex R24-P1-1 的新碼;
                #    R29 起也可能是子階段的 demucs 分軌 —— 種類寫在評審團的訊息裡)。
                ...
                # ⛔ 不可以寫死「來源快照」(Codex R29-P1-1):殘留可能是
                #    來源快照 / 上傳補正檔 / demucs 分軌,三種的處理與嚴重性不同。
                print("⛔ 評測完成但有暫存沒收乾淨(退出碼 4)——"
                      " 種類與完整路徑見上面評審團的輸出")
                ...
                if why:
                    ...
                    print("VERIFY_DIRTY 九柱完整、格式合格,但有暫存殘留未清 ——"
                          " 報告可用,請手動刪掉上面列出的目錄")
                    rc = 4
            elif r.returncode != 0:
