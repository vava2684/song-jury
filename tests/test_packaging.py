...
def test_批次與網頁版要接受退出碼2的不完整報告():
    ...
    for f in ("批次評測.py", "app.py"):
        src = (REPO / f).read_text(encoding="utf-8")
        # ⚠️ R25 起可讀的碼多了 4(報告已產出,只是來源快照沒收乾淨)——
        #    行為由 test_來源身分.py::test_批次遇到退出碼4要繼續讀報告 與
        #    test_rubric_pick.py::test_網頁版要處理快照殘留的退出碼 真的跑過;
        #    這裡只確認兩支用的是同一份「可讀退出碼」清單,沒有一邊漏改。
        assert "not in (0, 2, 4)" in src, \
            f"🔴 {f} 要把 exit 2/4(報告已產出)當可讀結果處理,不是當失敗丟掉"
