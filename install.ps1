...
if ($hasEnv) {
    ...
    #    (Codex R21-P2-2:只驗 rc/ok 時,「rc=4 但 kind=config_error」照樣被採信,
    #     使用者被導去改一個沒問題的環境變數;`recovered:"false"` 也會觸發警告)。
...
if ($VerifyModels) {
    if ($hasEnv) {
        ...
        switch ($vrc) {
            ...
            # ⛔ 4 是**評測有效、但有暫存沒清乾淨**(R24-P1-1 / R25-P1-1;
            #    種類可能是來源快照 / 上傳補正檔 / demucs 分軌,見 R29-P1-1)——
            #    與分軌線體檢那邊的 internal_error 4 是不同的命名空間,訊息要分得開:
            #    這裡不是「裝壞了」,是 TEMP 裡留了一份音訊要人去刪。
            4   { Warn "完整驗證:九柱與格式都合格,但**有暫存沒清乾淨**(退出碼 4)"
