...
⛔ 而且原因與 shape 必須**互相成立**(Codex R23-P1-2):`no_ffmpeg`/`probe_failed`
不可能附得出 shape;`unsupported_sample_fmt` 的 sample format 一定不在支援表、canonical
一定是空的;`unknown_multichannel_layout` 一定是 3 聲道以上、格式支援、算不出喇叭;
`decode_failed` 則是前面每一關都過了。少了這層關聯,「受支援格式卻沒算 PCM」只要
換個殼寫成降級宣告就能繞過 strict —— 裁判是獨立的,不能假設 JSON 一定來自這版產出端。
...
python -m pytest tests -q          # 461 條(約 6 分;含真子程序樹、兩支安裝器 × 兩個 PowerShell 引擎的退出碼矩陣、
...
```
[1/196] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/196] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/196] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/196] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 196 條真實缺陷全部會被測試抓到
```
