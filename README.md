...
`bash install.sh --verify-models` —— 它會實跑一遍九柱(首次會下載數 GB,很久),
用 `評審團.py` 的退出碼當判準(**0=完整評測、2=跑完但缺柱、其他=失敗**)。
...
> ⚠️ 誠實註記:範例檔是 64kbps mp3(展示用途)——聲學柱會**如實反映**壓縮造成的
> 高頻損失,分數比原始音檔低是正常的。這正是系統誠實的示範,不是 bug。
...
python -m pytest tests -q          # 179 條(約 15 秒;含真子程序樹行為測試)
...
```
[1/80] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/80] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/80] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/80] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 80 條真實缺陷全部會被測試抓到
```
