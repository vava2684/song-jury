...
`一鍵安裝.bat` 會把上面這些碼**原樣傳出**(它只在非 0 時 `pause` 給人看,不吃掉退出碼)。

(`評審團.py` 自己另有一套:**0=完整評測、2=報告已發布但缺柱、其他=失敗**;
`-VerifyModels` 的流程另有 **124=逾時、130=使用者中斷**。)
...
python -m pytest tests -q          # 280 條(約 60 秒;含真子程序樹、真的跑一次安裝器、比較器的行為測試)
...
```
[1/130] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/130] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/130] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/130] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 130 條真實缺陷全部會被測試抓到
```
...
三種都會單獨標示,不會混進通過率裡。Windows 上會有 3 條要 POSIX 語意
(symlink / 權限位元)的變異驗不了,CI 的 ubuntu 工作每次都會補驗。
