...
[2/9] Gemini 金鑰                ← ⭐ 全程唯一要你動手的一步,問完就能放著不管
[3/9] 建立量測環境 .venv
[4/9] 建立模型環境 .venv-ml(SongEval + Audiobox)
[5/9] 取得 SongEval 原始碼
[6/9] 鎖回 torch 版本
[7/9] 建立分軌環境 .venv-demucs
[8/9] 建立新耳朵環境 .venv-audition + 情緒詞典
...
> 不在工具資料夾裡 —— 同一台機器就算存了多份副本,互斥與金鑰冷卻也是共用的。
...
python -m pytest tests -q          # 136 條,約 3 秒
...
```
[1/60] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/60] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/60] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/60] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 60 條真實缺陷全部會被測試抓到
```
