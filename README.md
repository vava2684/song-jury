...
📁 **每一份暫存都有主人**(Codex R26-P1-1):上傳檔沒有(或非小寫)音檔副檔名時,
產品會複製一份補正副檔名的音訊 —— 那份也走同一條契約(用完刪掉、刪不掉就講出來
並用退出碼 4)。⭐ 補正檔只當**內容來源**,報告仍寫在你原本的檔案旁邊,不會被寫進
TEMP 再跟著暫存一起消失。網頁版的歌詞與情感弧線圖則放進**產品自己的受管目錄**
(`<狀態目錄>/web-tmp/`),每次新請求先回收超過 TTL(預設 1 小時,
`SONG_JURY_WEB_TMP_TTL` 可調)的舊產物 —— ⛔ 那可能是還沒公開的歌詞,
...
python -m pytest tests -q          # 496 條(約 6 分;含真子程序樹、兩支安裝器 × 兩個 PowerShell 引擎的退出碼矩陣、
...
```
[1/220] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/220] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/220] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/220] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 220 條真實缺陷全部會被測試抓到
```
