...
**[安裝](#安裝)** ·
**[用法](#用法)** ·
**[範例報告](#範例報告真的跑出來的不是示意圖)** ·
**[測試](#測試)** ·
**[誠實條款](#誠實條款)** ·
**[授權](#授權)**

> 🔧 想知道每條防線為什麼長成這樣(來源身分、暫存契約、退出碼、降級宣告)?
> 那些工程推導收在 **[`docs/設計契約.md`](docs/設計契約.md)**,不塞在這頁。
...
**同一份評測不可以投兩次票**:報告帶三種身分(`evaluation_id`、檔案 bytes 的雜湊、
**解碼後聲音**的雜湊),比較器據此拒絕「複製一份改個名字」或「換個容器再上場」
當成兩首歌 —— 那不必是惡意,整理檔案時順手複製就會發生,而結論會建立在同一份資料上。

評測一開始就把來源固定成一份私有快照(所有階段只讀它),每一份暫存都有主人、
用完就刪、刪不掉就講出完整路徑並用退出碼 4。

> 📐 這幾條防線各自為什麼長成這樣、每一版被什麼真實情境打破過(來源身分鏈、
> PCM 身分契約、暫存與租約、退出碼契約、降級宣告的關聯驗證)——
> 完整推導寫在 **[`docs/設計契約.md`](docs/設計契約.md)**。
> ⚠️ 誠實邊界也寫在那裡:這些防線處理的是**意外**造成的不一致,不是防篡改。
...
> 兩支安裝器共用同一份邏輯:會印出正在做什麼、暫時性失敗會再試一次
> (剛裝完那幾分鐘防毒還在掃新檔,失敗多半是假的),而且**分得出**
> 「缺套件」與「DLL/權限/快取損壞」—— 後者重裝 requirements 沒有用,不會叫你白做。
> 整段有預算上限(預設 900 秒,`SONG_JURY_DEMUCS_PROBE_TIMEOUT` 可調)。
> 按 Ctrl+C 是**立刻**停,不會清完檔案又繼續往下裝,TEMP 也不會留東西。
> 若是重試才成功,會給你一行黃字警告:那條線在這台機器上不穩,正式評分可能掉柱。
> (中斷處理、trap、暫存清理為什麼寫成現在這樣,見
> [`docs/設計契約.md`](docs/設計契約.md)。)
...
> 📄 單獨驗一份報告:`python 驗證報告.py 某某_評審團.json`。
> 要它當「本輪新產物」的證據,三個開關都要帶:
> `--newer-than <epoch> --require-contract --require-identity` ——
> ⛔ 相容可讀 ≠ 新產物證據。參數打錯、互斥開關同時給、`--newer-than nan`
> 都會 `VERIFY_BAD`,不會靜靜退成「通過」
> (每一條的來歷見 [`docs/設計契約.md`](docs/設計契約.md))。
...
> ⭐ **4 是「清理問題」不是「評測失敗」** —— 批次、網頁、`完整驗證.py`
> 三個下游都會照樣讀報告,只是把殘留路徑掛出來(完整驗證會印 `VERIFY_DIRTY`,
> ⛔ 絕不印 `VERIFY_OK`)。
> ⛔ 但 4 會蓋掉原本的 2,所以**完整性一律讀報告裡的 `pillar_totals.完整評測`**,
> 不可以因為「碼是 4」就當成完整評測。
> (每個下游各自怎麼處理、為什麼,見 [`docs/設計契約.md`](docs/設計契約.md)。)
...
> 🔒 `分軌線檢查.py` 另外寫一份 `--status-json` 給安裝器判斷**種類**,
> 由**獨立的** `狀態驗證.py` 驗過才採用(⛔ 驗證器不可以是被驗的那支程式自己),
> 與實際退出碼矛盾就整份忽略 —— 它只是診斷補充,永遠不能把失敗說成成功。
> (完整的欄位契約與威脅模型見 [`docs/設計契約.md`](docs/設計契約.md)。)
...
**MIT License** · © 2026 vava2684 —— `LICENSE` 是**純 MIT 全文**,沒有任何附加條款。

第三方模型與詞庫**一個都不在這個 repo 裡**,由使用者安裝時自官方源取得,各依其原授權;
完整清單見 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)。
⚠️ 要**商用**的話請先看那份清單:其中 SongEval 是 CC BY-NC-SA(非商用),
那條限制跟著它本身走,不會因為本專案是 MIT 就消失。
