...
`source_audio_pcm_sha256`(**解碼後**的聲音)。比較器據此拒絕「複製一份改個名字」
或「換個容器/改 metadata 再上場」當成兩首歌或兩個 take ——
那不必是惡意,整理檔案時順手複製就會發生,而結論會建立在同一份資料上。

輸出的 `source_identity.level` 誠實標出這批報告的證據強到哪裡:
`decoded-audio`(最強,換殼也認得出)→ `exact-file`(只認得同一個檔案,
產出端沒有 ffmpeg 時會這樣)→ `mixed` → `weak`(舊格式,只擋得掉完全相同的複製)。
⛔ 誠實邊界:lossy 重壓(320k→192k)會得到不同的聲音位元,那需要 acoustic
fingerprint,**本系統不做也不假裝做得到**。
...
> 不會被說成缺套件叫你重裝幾 GB)。輸出是即時的,不會整段悶到最後才吐。
...
python -m pytest tests -q          # 353 條(約 4 分;含真子程序樹、兩支安裝器 × 兩個 PowerShell 引擎的退出碼矩陣、
...
```
[1/153] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/153] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/153] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/153] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 153 條真實缺陷全部會被測試抓到
```
