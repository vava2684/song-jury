...
python -m pytest tests -q          # 442 條(約 6 分;含真子程序樹、兩支安裝器 × 兩個 PowerShell 引擎的退出碼矩陣、
...
```
[1/187] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/187] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/187] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/187] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 187 條真實缺陷全部會被測試抓到
```
