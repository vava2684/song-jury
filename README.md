...
**同一份評測不可以投兩次票**:報告會帶 `evaluation_id` 與整首音檔的
`source_audio_sha256`,比較器據此拒絕「複製一份、改個名字」當成兩首歌或兩個 take ——
那不必是惡意,整理檔案時順手複製就會發生,而結論會建立在同一份資料上。
輸出的 `source_identity` 會誠實標出這批報告的防線強度(舊格式報告沒有這兩個欄位,
只剩「內容完全相同」那一層,會標成 `weak`)。
...
python -m pytest tests -q          # 312 條(約 2 分;含真子程序樹、真的跑兩支安裝器的退出碼矩陣、比較器行為)
...
```
[1/142] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/142] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/142] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/142] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 142 條真實缺陷全部會被測試抓到
```
