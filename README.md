...
> · **`channel_layout` 不進雜湊**:那個字串是**容器/探測器**的描述,不是聲音本身 ——
>   實測同一段 PCM 裝 WAV 被寫成 `unknown`、裝 MOV/CAF 寫成 `mono`(Codex R21-P2-1)。
>   聲道**數量**已經在身分裡;完整結構另外寫在 `source_audio_pcm_shape` 供稽核;
...
```
[1/180] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/180] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/180] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/180] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 180 條真實缺陷全部會被測試抓到
```
