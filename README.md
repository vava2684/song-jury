...
python -m pytest tests -q          # 63 條,約 0.5 秒
...
```
[1/11] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/11] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/11] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/11] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 11 條真實缺陷全部會被測試抓到
```
