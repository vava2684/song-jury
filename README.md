...
交給呼叫端、由 main 的最外層 finally 清掉,demucs 自己失敗時就地清(R27-P1-2)。網頁版的歌詞與情感弧線圖則放進**產品自己的受管目錄**(`<狀態目錄>/web-tmp/`),
帶**租約**:請求開始時放 `.active`,產物交給前端後解除 —— 回收只清「已完成且過了
保留期」(預設 1 小時,`SONG_JURY_WEB_TMP_TTL`)的目錄;還 active 但超過**放棄期**
(預設 6 小時,`SONG_JURY_WEB_ABANDON_AGE`)才當成被中斷的孤兒回收。
...
python -m pytest tests -q          # 501 條(約 6 分;含真子程序樹、兩支安裝器 × 兩個 PowerShell 引擎的退出碼矩陣、
...
```
[1/226] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/226] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/226] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/226] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 226 條真實缺陷全部會被測試抓到
```
