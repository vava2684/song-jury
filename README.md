...
> · **重新取樣或改聲道數**的版本會被當成不同來源 —— 解碼身分刻意保留原始結構,
>   因為「先轉成 44.1k 立體聲再比」是**多對一**的,實測會把 48k 單聲道與
>   由它轉出的 44.1k 雙單聲道判成同一個來源(Codex R19-1);
> · **樣本格式也保留**:浮點來源解成 f64le、整數來源才用 s32le ——
...
python -m pytest tests -q          # 392 條(約 6 分;含真子程序樹、兩支安裝器 × 兩個 PowerShell 引擎的退出碼矩陣、
...
```
[1/172] ✅ 抓到了:切窗漏掉最後一個完整窗(40s 只分析 1 個窗)
[2/172] ✅ 抓到了:Gemini 分數不夾範圍(M1:99 → 990/100)
[3/172] ✅ 抓到了:Gemini 總分取錯鍵名(整關被靜默丟掉)
[4/172] ✅ 抓到了:快取不驗身分(同名不同曲會讀到別首歌的分軌)
...
  ✅ 172 條真實缺陷全部會被測試抓到
```
